    ...
    public void render(UIContext context)
    {
        ...
        if (!forms.isEmpty() && this.category.visible.get())
        {
            for (Form form : forms)
            {
                ...
                if (isSelected)
                {
                    context.batcher.box(cx, cy, cx + CELL_WIDTH, cy + CELL_HEIGHT, Colors.A50 | BBSSettings.accentColor());
                    context.batcher.outline(cx, cy, cx + CELL_WIDTH, cy + CELL_HEIGHT, Colors.A50 | BBSSettings.accentColor(), 2);
                }
